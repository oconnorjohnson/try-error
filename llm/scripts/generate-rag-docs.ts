...
class RagDocGenerator {
      ...
      ts.forEachChild(sourceFile, (node) => {
        ...
        if (ts.isVariableStatement(node)) {
          ...
          if (declaration.initializer && ts.isArrowFunction(declaration.initializer)) {
            const funcDoc = this.extractArrowFunctionInfo(declaration, sourceFile);
            if (funcDoc) {
              functions.push(funcDoc);
            }
          }
  ...
  private extractFunctionInfo(node: ts.FunctionDeclaration, sourceFile: ts.SourceFile): FunctionDoc | null {
  ...
  private extractArrowFunctionInfo(node: ts.VariableDeclaration, sourceFile: ts.SourceFile): FunctionDoc | null {
  ...
  private extractParameters(signature: ts.Signature): ParameterDoc[] {
    return signature.parameters.map(param => {
      const type = this.checker.getTypeOfSymbolAtLocation(param, param.valueDeclaration!);
  ...
  private findExamples(functionName: string): string[] {
    ...
    if (fs.existsSync(docsPath)) {
      ...
      for (const file of mdFiles) {
        ...
        const regex = new RegExp(`\`\`\`(?:typescript|javascript|ts|js)[^\\n]*\\n([^\\n]*${functionName}[^\\n]*\\n[^\\`]*)\\n\`\`\``, 'g');
        let match;
  ...
  private findTestUsage(functionName: string): string[] {
    ...
    const testFiles = this.sourceFiles.filter(sf => 
      sf.fileName.includes('.test.') || sf.fileName.includes('.spec.')
    );
  ...
  private getPerformanceData(functionName: string): PerformanceData | undefined {
    // Run benchmarks if available
    try {
      const benchmarkPath = path.join(this.projectPath, 'benchmark', `${functionName}.bench.ts`);
...
## Parameters
${func.parameters.map(p => `- **${p.name}**: \`${p.type}\` ${p.optional ? '(optional)' : ''} - ${p.description || 'No description'}`).join('\n')}
...
### Dependencies
${func.dependencies.length > 0 ? func.dependencies.map(d => `- ${d}()`).join('\n') : 'No internal dependencies'}
...
### From Documentation
${func.examples.length > 0 ? func.examples.map((ex, i) => `
#### Example ${i + 1}
\`\`\`typescript
${ex}
\`\`\`
...
### From Tests
${func.usageInTests.length > 0 ? func.usageInTests.slice(0, 3).map((usage, i) => `
\`\`\`typescript
${usage}
\`\`\`
`).join('\n') : 'No test usage found'}
...
`;
  ...
  private inferTags(func: FunctionDoc): string {
    ...
    if (func.parameters.some(p => p.name === 'context')) tags.push('context-aware');
...
## Functions
${Object.keys(docs).map(name => `- [${name}()](./${name}-deep-dive.md)`).join('\n')}

Generated on: ${new Date().toISOString()}
`;
  
  fs.writeFileSync(path.join(outputPath, 'index.md'), indexContent);
  console.log(`\nGenerated ${Object.keys(docs).length} documentation files in ${outputPath}`);
}
